...
// ══════════════════════════════════════════════════════════════════════════════
// SPOTME RADAR – JAVASCRIPT TEMPLATE
// ══════════════════════════════════════════════════════════════════════════════
//
// ANLEITUNG: Neuen Spot erstellen
// ─────────────────────────────────────────────────────────────────────────────
// 1. Diese Datei kopieren → umbenennen: spot-[SPOT_ID].js
//    Beispiele: spot-club.js, spot-sport.js, spot-reisen.js
//
// 2. Folgende Stellen anpassen (markiert mit [ANPASSEN]):
//    a) SPOT       = eindeutiger Namespace (Kleinbuchstaben, kein Leerzeichen)
//    b) SPOT_CHIPS = Filter-Felder die dieser Spot nutzt
//    c) applyFilters() = Filter-Logik passend zu den Chips
//    d) renderBadges() = Badge-Darstellung in Karten + Detail-Modal
//
// 3. In spot-template.html ebenfalls anpassen:
//    - Farben (CSS-Variablen)
//    - Logo-Text
//    - Filter-Chips im HTML
//    - JS-Datei referenz
//
// NICHT ANFASSEN: Alles unterhalb von "── KERN (nicht ändern) ──"
// ══════════════════════════════════════════════════════════════════════════════

// ══════════════════════════════════════════════════════════════════════════════
// [ANPASSEN A] SERVER & SPOT-NAMESPACE
// ══════════════════════════════════════════════════════════════════════════════
const API  = 'https://spotme-pg-test.onrender.com/api'; // Server-URL (gleich für alle Spots)
const SPOT = '[SPOT_ID]'; // [ANPASSEN] z.B. 'club', 'sport', 'reisen', 'gay', 'general'

// ══════════════════════════════════════════════════════════════════════════════
// [ANPASSEN B] SPOT-SPEZIFISCHE FELDER
// Diese Felder werden im POST /api/profile mitgesendet.
// Nur Felder definieren die dieser Spot wirklich nutzt.
// ══════════════════════════════════════════════════════════════════════════════
//
// Gay-Spot:     orientation, role, trans, cross
// Club-Spot:    genre (techno/house/rnb/hiphop/live/openair)
// Sport-Spot:   sport (fussball/tennis/laufen/radfahren/fitness/klettern)
// Reisen-Spot:  reiseart (backpacker/städtereise/strand/berge/roadtrip)
// General-Spot: category (sport/musik/essen/gaming/reisen/kultur/natur)
//
// Alle anderen Felder (name, age, region, bio) sind immer Pflicht/optional.
  ...
  try {
    if (isPublished) {
      ...
      const payload = {
        ...
        // ── [ANPASSEN E] Spot-spezifische Profilfelder im POST-Body ──
        // Gay:     orientation: myProfile.orientation || null, role: myProfile.role || null, trans: !!myProfile.trans, cross: !!myProfile.cross,
        // Club:    genre: myProfile.genre || null,
        // Sport:   sport: myProfile.sport || null,
        // General: category: myProfile.category || null,
