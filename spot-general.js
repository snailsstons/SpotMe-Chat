...
async function togglePublish() {
  ...
  try {
    if (isPublished) {
      await fetch(API + '/profile/' + myCode, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: myToken }) });
      isPublished = false;
      ...
      const payload = {
        ...
        category: myProfile.category || null, bio: myProfile.bio || null,
...
function applyFilters() {
  ...
  filtered = allProfiles.filter(p => {
    ...
    // Kategorie-Filter: wenn Chips aktiv, muss Profil mindestens eine davon haben
    const CATEGORIES = ['sport','musik','essen','gaming','reisen','kultur','natur','sonstiges'];
    const catChips = chips.filter(f => CATEGORIES.includes(f));
...
function renderList() {
  ...
  listEl.innerHTML = filtered.map(p => {
    ...
    let badges = '';
    if (p.category) {
      const catLabel = { sport:'⚽ Sport', musik:'🎵 Musik', essen:'🍕 Essen & Trinken', gaming:'🎮 Gaming', reisen:'✈️ Reisen', kultur:'🎭 Kultur', natur:'🌿 Natur', sonstiges:'💬 Sonstiges' }[p.category] || p.category;
      badges += `<span class="badge badge-role">${esc(catLabel)}</span>`;
    }
...
function showProfileDetail(profile) {
  ...
  let badges = '';
  if (profile.category) {
    const catLabel = { sport:'⚽ Sport', musik:'🎵 Musik', essen:'🍕 Essen & Trinken', gaming:'🎮 Gaming', reisen:'✈️ Reisen', kultur:'🎭 Kultur', natur:'🌿 Natur', sonstiges:'💬 Sonstiges' }[profile.category] || profile.category;
    badges += `<span class="badge badge-role">${esc(catLabel)}</span>`;
  }
